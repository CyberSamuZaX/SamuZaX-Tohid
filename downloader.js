...
rows: srh
    ...
    const listMessage = {
caption: ` KPST-MD ♥ 💂 MODWHATSAPP-DL ⚜️`,
image : { url: config.LOGO },	    
footer: config.FOOTER,
...
rows: srh
    ...
    const listMessage = {
text: `⚜️ KPST-MD ♥ MODWHATSAPP-DL ⚜️\n\n🚀 Title :- ${title}\n📄 Date :- ${date}`,    
footer: config.FOOTER,
title: 'Result from Mod Whatsapp. 📲',
buttonText: '*🔢 Reply below number*',
sections
...
try{
    ...
    gis(q, async (error, result) => {
        ...
        for (let i = 0; i < topImages.length; i++) {
          ...
          let Message = {
              image: { url: imageUrl },caption: `*-------「 KPST-MD ♥ 💂 GIMAGE SEARCH 」-------*\n🤠 *Query* : ${q}\n\n🔗 *Image ${i + 1} Url* : ${imageUrl}`,
           }

//let senda = await conn.sendMessage(from, { document: {url: imageUrl },fileName: 'image' + '.jpg', mimetype: 'image/jpeg' ,caption: `*-------「 KPT AI GIMAGE SEARCH 」-------*\n🤠 *Query* : ${q}\n\n🔗 *Image ${i + 1} Url* : ${imageUrl}`,}, { quoted: mek })  
...
try{
    ...
    gis(q, async (error, result) => {
        ...
        for (let i = 0; i < topImages.length; i++) {
            const imageUrl = topImages[i].url
          let Message = {
              document: { url: imageUrl },fileName: 'image' + '.jpg', mimetype: 'image/jpeg' ,caption: `*-------「 KPST-MD ♥ 💂 GIMAGE SEARCH 」-------*\n🤠 *Query* : ${q}\n\n🔗 *Image ${i + 1} Url* : ${imageUrl}`,
           }
...
` } ,{ quoted: mek })
  ...
  reply('*📥 𝙐𝙋𝙇𝙊𝘼𝘿𝙀𝘿 𝘽𝙔 KPST-MD ♥ 💂 𝙊𝙒𝙉𝙀𝙍*')
l(e)
}
})
//=================================APK COMMANDS=====================================
